...
## v0.14.1 — macOS/BSD nm互換修正

- **bugfix**: macOS CIでtiny_functionチェックのテストが失敗する問題を修正
  - nmのシンボル名先頭`_`プレフィックスを自動除去
  - サイズ列がないBSD nm形式に対応（アドレス差からサイズ推定）
  - コンパイラ内部ラベル（ltmp等）を除外

## v0.4.0 → v0.14.0 更新内容

### v0.14.0 — リンケージ解析＆オブジェクトサイズ検出

- **tiny_function**: コンパイル後オブジェクトサイズ5バイト以下の関数を検出（gcc -O2 + nm -S）
  - gcc未使用環境ではソースコード解析にフォールバック
- **linkage**: extern宣言に対応する定義が同一ファイル内に存在するか検証
- **undefined_call**: 宣言・定義なしの関数呼び出しを検出（ヘッダinclude漏れ）
- テスト122件に増加
